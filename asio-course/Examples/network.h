...
private:
    ...
    volatile std::atomic_int m_error_state;
...
private:
    ...
    void StartTimer();
    void StartError(const std::error_code & error);
    void DispatchSend(std::vector< uint8_t > buffer);
    void DispatchRecv(int32_t total_bytes);
    void DispatchTimer(const std::error_code & error);
    void HandleConnect(const std::error_code & error);
    void HandleSend(const std::error_code & error, std::list< std::vector< uint8_t > >::iterator itr);
    void HandleRecv(const std::error_code & error, int32_t actual_bytes);
...
private:
    ...
    virtual void OnTimer(const std::chrono::microseconds & delta) = 0;
...
private:
    ...
    void HandleTimer(const std::error_code & error);
    void HandleAccept(const std::error_code & error, std::shared_ptr< Connection > connection);
...
private:
    ...
    volatile std::atomic_int m_shutdown;
