<?php
	...
	function do_parents($db, $gid)
	{
		...
		$result = $db->query(
			"select 
				...
				F.gid as family_gid, 
				father_gid, 
				FN.first_name||' '||FN.surname as FatherName,
				CR.frel0,
				mother_gid,
				MN.first_name||' '||MN.surname as MotherName,
				CR.mrel0
			from child_ref CR
			inner join family F
				on F.gid = CR.gid
		...
		unset($row);
		$result = $db->query(
			"select CR2.sortval, CR2.child_gid,
				N.first_name||' '||N.surname as Name
			from child_ref CR
	...
	function do_father($db, $gid, $top)
	{
		...
		$result = $db->query(
			"select 
				father_gid, FN.first_name||\" \"||FN.surname as name
			from child_ref CR
			inner join family F
				on F.gid = CR.gid
			left join name FN
	...
	function do_mother($db, $gid, $top)
	{
		...
		$result = $db->query(
			"select 
				mother_gid, MN.first_name||\" \"||MN.surname as name
			from child_ref CR
			inner join family F
				on F.gid = CR.gid
			left join name MN
